...
系统在追踪线程中读取IMU实时状态，并构建前馈补偿项。以偏航轴为例，控制输出如式\eqref{eq:imu-feedforward}所示，该结构参考\cite{Han2009ADRC,Han2008ADRCBook}中自抗扰控制思想，在传统PID外环基础上叠加角速度与加速度前馈项：
...
其中，\(u_{pid}\)为视觉误差闭环输出，\(\omega_{yaw}\)为角速度项，\(\dot{\omega}_{yaw}\)为角加速度近似项，\(a_x\)为加速度补偿项。该结构能够在平台突加速或抖动时提前修正执行量，降低纯视觉闭环的相位滞后影响\cite{Han2002ADRCChinese,Mahony2008IMU}。
...
根据参考目录下\texttt{edc\_opencv/EDC\_Opencv.py}的实现，视觉端采用“三线程+双队列”结构：
...
在灰度数据的处理流程上，工程对8路原始ADC输出进行了四级处理以提升控制丝滑度：①\textbf{均值滤波}，对每路做多次采样平均，降低电气噪声；②\textbf{归一化计算}，以黑白基准值为端点，将各路输出映射到$[0,1]$区间，消除传感器个体差异和安装高度变化的影响；③\textbf{加权偏差估计}，对8路归一化值按位置权重相加求得横向偏差量，近中心传感器权重小、远端权重大，提高边缘感知灵敏度；④\textbf{一阶滞后滤波}，对最终偏差量施加低通滤波$y_k = (1-\alpha)\,y_{k-1}+\alpha\,u_k$，防止偏差突变引起的云台/底盘抖动，保证整车运行平稳。
...
在执行层，系统先依据差速模型\cite{Siegwart2011robots,Cai2015Robotics}进行左右轮目标速度分解，再结合编码器反馈完成速度闭环控制。左右轮目标转速分解关系如式\eqref{eq:diff-kinematics}所示：
