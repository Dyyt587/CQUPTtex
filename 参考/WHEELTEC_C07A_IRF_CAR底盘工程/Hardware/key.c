...
uint8_t keyValue_2(void)
{
    return ( DL_GPIO_readPins(KEY_USER_PORT,KEY_USER_PIN_8_PIN) & KEY_USER_PIN_8_PIN ) > 0 ? 0:1;
}
...
UserKeyState_t key_scan_2(uint16_t freq)
{
    ...
    if(check_once&&0 == keyValue_2()) check_once=0; //完成扫描后按键被弹起，则开启下一次扫描

    if(1==keyValue_2()&&check_once==0)//按键扫描
    {
        press_flag=1;//标记被按下1次
        long_press_time++;		  
    }
