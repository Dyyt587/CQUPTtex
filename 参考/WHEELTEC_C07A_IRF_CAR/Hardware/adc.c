...
uint8_t is_jianche=0;			//是否开启提前检测
uint8_t jiaozhun=0;				//0:不开启校准，1：开启白色校准  2：开启黑色校准
...
uint16_t Get_adc_value(void)
{
        ...
        //如果当前状态为正在转换中则等待转换结束
        //获取数据
        gAdcResult = DL_ADC12_getMemResult(ADC12_VOLTAGE_INST, ADC12_VOLTAGE_ADCMEM_0);
        //清除标志位
        gCheckADC = false;
...
//0:不存在拐点
//1：存在拐点
uint8_t gray_is_turn(const uint16_t normal_value[8])
{
	uint32_t sum=0;
	uint16_t average=0;
	uint8_t result=0;
	...
	average=sum/8;
	if(average<3000)	//可能处于拐点
	{
		uint8_t count=0;
		for(uint8_t i=0;i<4;i++)
		{
			if(normal_value[i]<2000)
			{
				count++;
			}
		}
		if(count>=3&&is_jianche==1)	//开始检测为1时，才进入，不然不进入
		{
			result=1;
		}
	}
	else if(average>=4000)	//处于全白点
	{
		result=2;
	}
	//printf("arv:%d\n",sum);
...
//计算灰度值pid期望
int gray_count_value(const uint16_t value[8])
{
	int weight_value=0;	//最终加权值
	int weight[8]={-30,-20,-10,-5,5,10,20,30};	//加权数组
	uint16_t inverse_value[8]={0};	//数字取反
	for(uint8_t i=0;i<8;i++)
	{
		inverse_value[i]=4096-value[i];
	}	//大小倒序
	for(uint8_t i=0;i<8;i++)
	{
		weight_value+=weight[i]*inverse_value[i];
	}
	return weight_value/100;
//	if(value[3]!=0)	return -8;
//	else if(value[4]!=0)	return 8;
//	else if(value[2]!=0) return -12;
//	else if(value[5]!=0) return 12;
//	else if(value[1]!=0) return -16;
//	else if(value[6]!=0) return 16;
//	else if(value[0]!=0) return -28;
//	else if(value[7]!=0) return 28;
//	else	return 0;
}
