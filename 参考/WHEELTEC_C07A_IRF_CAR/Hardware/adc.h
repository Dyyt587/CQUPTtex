...
/*************************** 硬件抽象层配置 ****************************/
// GPIO地址切换宏定义
#define Switch_Address_0(i) ((i)?(DL_GPIO_setPins(GRAY_ADDRESS_PORT,GRAY_ADDRESS_PIN_AD0_PIN)) : (DL_GPIO_clearPins(GRAY_ADDRESS_PORT,GRAY_ADDRESS_PIN_AD0_PIN)))// 地址位0控制

#define Switch_Address_1(i) ((i)?(DL_GPIO_setPins(GRAY_ADDRESS_PORT,GRAY_ADDRESS_PIN_AD1_PIN)) : (DL_GPIO_clearPins(GRAY_ADDRESS_PORT,GRAY_ADDRESS_PIN_AD1_PIN)))// 地址位1控制

#define Switch_Address_2(i) ((i)?(DL_GPIO_setPins(GRAY_ADDRESS_PORT,GRAY_ADDRESS_PIN_AD2_PIN)) : (DL_GPIO_clearPins(GRAY_ADDRESS_PORT,GRAY_ADDRESS_PIN_AD2_PIN)))// 地址位2控制

// ADC值获取接口宏定义 需要自己根据单片机完成对应位数的ADC采样函数
#define Get_adc_of_user() Get_adc_value()  // 用户自定义ADC读取函数
/**********************************************************************/
...
extern No_MCU_Sensor sensor;	//全局变量
extern uint8_t is_jianche;
extern uint8_t jiaozhun;	//校准
