...
static int decode_hipnuc(hipnuc_raw_t *raw)
{
    ...
    /* checksum */
    hipnuc_crc16(&crc, raw->buf, (CH_HDR_SIZE-2));				//将前四个字节进行校验
    hipnuc_crc16(&crc, raw->buf + CH_HDR_SIZE, raw->len);	//将数据段进行校验
...
{
    /* synchronize frame */
    if (raw->nbyte == 0)	//循环检测帧头
    {
        if (!sync_hipnuc(raw->buf, data))
            return 0;
        raw->nbyte = 2;	//检测到帧头
				//LOG_D("sucess receive head");
        return 0;
    }

    raw->buf[raw->nbyte++] = data;

    if (raw->nbyte == CH_HDR_SIZE)	//数据长度校验，若长度大于数据帧最大长度，丢弃
    {
...
static rt_thread_t thread_hipnuc = RT_NULL;  
#define USART5_NAME  "uart5"
static rt_device_t serial5;										//串口设备句柄
static struct serial_configure uart5_config = RT_SERIAL_CONFIG_DEFAULT;  /* 初始化配置参数 */
static rt_sem_t hipnuc_rx_sem= RT_NULL;			//hipnuc接收数据信号量

hipnuc_raw_t hipnuc_raw = {0};	//原始数据结构体
...
static void thread_hipnuc_entry(void *parameter)
{
...
//      LOG_D("%s\r\n", log_buf);
		...
		if(rt_sem_take(hipnuc_rx_sem,RT_WAITING_FOREVER)==RT_EOK)
		{
			hipnuc_dump_packet(&hipnuc_raw, log_buf, sizeof(log_buf));
//      LOG_D("parse ok, frame len:%d\r\n", hipnuc_raw.len);
//      LOG_D("%s\r\n", log_buf);
		}
    //rt_thread_mdelay(5);	//延时太长会导致数据读出慢
	}
}
static void hipnuc_print(int argc, char **argv)
{
//		hipnuc_dump_packet(&hipnuc_raw, log_buf, sizeof(log_buf));
//    LOG_D("parse ok, frame len:%d\r\n", hipnuc_raw.len);
//    LOG_D("%s\r\n", log_buf);
	LOG_D("Roll:%.5f,Pitch:%.5f,Yaw:%.5f",hipnuc_raw.hi91.roll,hipnuc_raw.hi91.pitch,hipnuc_raw.hi91.yaw);
}MSH_CMD_EXPORT(hipnuc_print, MS4010 motor test commands);
//			hipnuc_dump_packet(&hipnuc_raw, log_buf, sizeof(log_buf));
//      LOG_D("parse ok, frame len:%d\r\n", hipnuc_raw.len);
//      LOG_D("%s\r\n", log_buf);
...
static rt_err_t usart_rx_callback(rt_device_t dev, rt_size_t size)
{
	...
	rt_device_read(dev,0,rx_data,size);
	for(uint8_t i=0;i<size;i++)
	{
		if(hipnuc_input(&hipnuc_raw, rx_data[i]))
		{
			flag=1;
			
			rt_sem_release(hipnuc_rx_sem);
		}
	}
	//LOG_D("%d",size);
	return RT_EOK;
...
int hipnuc_init(void) {
  ...
  thread_hipnuc =
      rt_thread_create("thread_hipnuc", thread_hipnuc_entry, RT_NULL, 2048, 10, 1);
	

	    /* 查找串口设备 */
    serial5 = rt_device_find(USART5_NAME);
    if (!serial5)
    {
        LOG_D("find %s failed!\n",USART5_NAME);
        return RT_ERROR;
		...
		//修改串口配置参数 
		uart5_config.baud_rate = BAUD_RATE_921600;        // 修改波特率为 921600
		uart5_config.data_bits = DATA_BITS_8;           // 数据位 8
		uart5_config.stop_bits = STOP_BITS_1;           // 停止位 1
		uart5_config.rx_bufsz     = BSP_UART5_RX_BUFSIZE;       // 修改缓冲区 
		uart5_config.tx_bufsz			= BSP_UART5_TX_BUFSIZE;
		uart5_config.parity    = PARITY_NONE;           // 无奇偶校验位
		
		if(rt_device_control(serial5, RT_DEVICE_CTRL_CONFIG, &uart5_config)!=RT_EOK)
		{
			LOG_D("IMU Change BAUD ERROR");
		}
		
		hipnuc_rx_sem = rt_sem_create("hipnuc_rx_sem", 0, RT_IPC_FLAG_FIFO);	//创建信号量
		if(hipnuc_rx_sem==RT_NULL)
		{
			LOG_D("Hipnuc_RX_Sem Create ERROR");
		}
    ...
    rt_device_set_rx_indicate(serial5, usart_rx_callback);
		...
		if (thread_hipnuc != RT_NULL) {
			rt_thread_startup(thread_hipnuc);
		}
	return 0;
}
INIT_ENV_EXPORT(hipnuc_init);

/**
 * @brief 获取当前陀螺仪数据
 */
int hipnuc_get_gyro_data(float *gyro_x, float *gyro_y, float *gyro_z)
{
    ...
    /* 检查数据是否有效 */
    if (hipnuc_raw.hi91.tag != HIPNUC_ID_HI91)
    {
        return -1;
    }

    *gyro_x = hipnuc_raw.hi91.gyr[0];  // X轴陀螺仪数据
    *gyro_y = hipnuc_raw.hi91.gyr[1];  // Y轴陀螺仪数据  
    *gyro_z = hipnuc_raw.hi91.gyr[2];  // Z轴陀螺仪数据
 ...
 * @brief 获取当前欧拉角数据
 */
int hipnuc_get_euler_data(float *roll, float *pitch, float *yaw)
{
    ...
    *roll = hipnuc_raw.hi91.roll;   // 横滚角
    *pitch = hipnuc_raw.hi91.pitch; // 俯仰角
    *yaw = hipnuc_raw.hi91.yaw;     // 偏航角
